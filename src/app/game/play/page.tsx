...
export default function PlayPage() {
  ...
  useEffect(() => {
    ...
    const arr: Player[] = Array.from({ length: playersCount }, (_, i) => ({ id: i + 1, role: "normal" as Role, word: undefined, revealed: false }));
    // assign impostors (fingidazo) randomly - ensure at least 1
  ...
  function nextPlayer() {
    if (current + 1 < players.length) {
      ...
      router.push(`/game/match?cat=${encodeURIComponent(cat)}&players=${playersCount}&impostors=${impostorsCount}&secret=${encodeURIComponent(secret)}&fing=${encodeURIComponent(fingParam)}`);
    }
  }
  ...
  return (
    <main style={{ background: "#000" }} className="w-full h-screen flex items-center justify-center">
      ...
      <div className="wrap">
        ...
        <div style={{textAlign:'center',marginBottom:8}} className="hint">Jugador {current + 1} de {players.length} — toca la tarjeta para revelar</div>
              ...
              <div style={{textAlign:'center',padding:12}}>
                {curPlayer.role === 'fingidazo' ? (
                  <>
                    ...
                    <div className="role" style={{marginTop:8}}>Eres el Fingidazo — intenta descubrir la palabra sin que te descubran</div>
                  </>
                ) : (
                  <>
                    <div className="title">JUGADOR</div>
                    <div className="role" style={{marginTop:8}}>Palabra: <strong>{curPlayer.word}</strong></div>
                  </>
                )}
              </div>
            ) : (
        ...
        <div className="controls">
          <button className="btn" onClick={revealCurrent} disabled={!curPlayer || curPlayer.revealed}>Revelar</button>
          <button className="btn" onClick={nextPlayer} disabled={!curPlayer || !curPlayer.revealed}>{current + 1 < players.length ? 'Siguiente jugador' : 'Ir al juego'}</button>
        </div>
      </div>
    </main>
  );
